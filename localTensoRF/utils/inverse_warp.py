...
def inverse_warp2(img, depth, pose, rotation_mode='euler', padding_mode='zeros'):
    """
    ...
    intrinsics = torch.eye(3, 3).unsqueeze(dim=0)
...
def inverse_warp(img, depth, ref_depth, pose_mat, padding_mode='zeros'):
    """
    Inverse warp a source image to the target image plane.
    Args:
        img: the source image (where to sample pixels) -- [B, 3, H, W]
