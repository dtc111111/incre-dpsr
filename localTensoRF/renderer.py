...
from utils.utils import (compute_depth_loss, get_fwd_bwd_cam2cams, get_pred_flow, rgb_ssim, visualize_depth, draw_poses, rgb_lpips)
...
def render(
    ...
    if test:
        ...
        poses_mtx = poses_mtx[start:] #poses_mtx和idxs大小
    ...
    for i, idx in tqdm(enumerate(idxs)):
        ...
        if test:
            ...
            lpips = rgb_lpips(gt_rgb.numpy(), rgb_map.numpy(), "vgg" ,poses_mtx.device)
            metrics[fbase] = {
                "mse": mse,
                "ssim": ssim,
                "lpips": lpips,
