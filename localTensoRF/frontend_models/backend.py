...
class Backend:
    ...
    def ba(self, t_start, t_end, steps, graph, nms, radius, thresh, max_factors, t_start_loop=None, loop=False, motion_only=False):
        """ main update """  ### 完整的full ba过程在src/geom/ba.py的def BA里，这个顺序和DROID-SLAM一样。backend里的ba是对图的维护
        if t_start_loop is None or not loop:
            t_start_loop = t_start #整个ba对应的就是论文3.1节里局部covis和loop closure的两部分，因此t_s_l就是t_s，最远回溯到回环检测的开头
        assert t_start_loop >= t_start, f'short: {t_start_loop}, long: {t_start}.'

        ilen = (t_end - t_start_loop)
        jlen = (t_end - t_start)
        ix = torch.arange(t_start_loop, t_end) #从小到大排列 [t_start_loop, t_s_l+1, ..., t_end]
        jx = torch.arange(t_start, t_end) #[t_start, t_s+1, ..., t_end]

        ii, jj = torch.meshgrid(ix, jx, indexing='ij') #ii为ix转置，重复jlen次，维度为（ilen*jlen）；jj为jx重复ilen次，维度也为（ilen*jlen）
        ii = ii.reshape(-1) #展平为一维
        jj = jj.reshape(-1)

        d = self.video.distance(ii, jj, beta=self.beta) #distance定义见depth_video.py，最终指向covis matrix
        rawd = deepcopy(d).reshape(ilen, jlen) 
        d[ii - radius < jj] = np.inf #将d中满足ii-r<jj的元素设为无穷大
        d[d > thresh] = np.inf
        d = d.reshape(ilen, jlen)

        es = [] #要维护的边列表
        # build edges within local window [i-rad, i] 对应的是论文里图2(b)局部边列表维护的一部分
        ...
        while len(ix) > 0:  
            ...
            if loop:
                sub_es = []
                num_loop = 0
                for si in range(max(i-n_neighboring, t_start_loop), min(i+n_neighboring+1, t_end)): #(i-1, i+1)
                    for sj in range(max(j-n_neighboring, t_start), min(j+n_neighboring+1, t_end)): #(j-1, j+1)
        ...
        ii, jj = torch.tensor(es, device=self.device).unbind(dim=-1) #unbind表示移除最后一个维度

        graph.add_factors(ii, jj, remove=True) #graph在实例化时应是FactorGraph类，定义见src/factor_graph.py

        edge_num = len(graph.ii)

        graph.update_lowmem( #update的定义在src/factor_graph.py的FactorGraph类定义里
        ...
        return edge_num
    #总结：和全局ba有关的部分：1）backend.py，图的维护与优化过程；2）factor_graph.py，定义了factor graph及其相关操作；3）depth_video.py，定义了对视频帧所作的操作（如计算距离、提取pose、reproject）；4）ba.py，定义了更新操作
    #需要的源代码和库：以上4个+Lietorch+droid_backends（可能需要重新编译）
