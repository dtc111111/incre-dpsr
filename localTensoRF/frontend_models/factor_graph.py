...
class FactorGraph:
    ...
    @torch.cuda.amp.autocast(enabled=False) #混合精度
    def update_lowmem(self, t0=None, t1=None, iters=2, use_inactive=False, EPS=1e-7, steps=8, max_t=None, ba_type='dense', motion_only=False):
        ...
        corr_op = AltCorrBlock(self.video.fmaps[sel_index].view(1, num*rig, ch, ht, wd)) #ACB定义在src/modules/corr.py里
        ...
        for step in range(steps):
            with torch.cuda.amp.autocast(enabled=False):
                # [batch, N, h, w, 2]
                coords1, mask = self.video.reproject(self.ii, self.jj) #reproject对应的就是E(G,d)那个error函数，需要最小化；定义在depth_video.py里。从两个边里提取出对应的pose，做reprojection
                motion = torch.cat([coords1 - self.coords0, self.target - coords1], dim=-1)
                motion = motion.permute(0, 1, 4, 2, 3).clamp(-64.0, 64.0)
